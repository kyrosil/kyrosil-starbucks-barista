// --- Global Değişken Tanımları (SENİN KODUNDAN ALINDI) ---
let canvas, ctx;
// Giriş ekranı elementleri bu kodda başlangıçta atanmıyor, DOMContentLoaded bekleniyor
...
let assetsReady = false; // Senin kodunda vardı, buton durumu için kullanılıyordu, koruyorum.
...
// --- Sabit Veriler (GÜNCELLENDİ - İngilizce ve nameKey) ---
const texts = {
    TR: {
...
function tryStartGame() {
    if (gameState !== 'LOADING') { console.warn("tryStartGame tekrar çağrıldı, state:", gameState); return; } // Sadece yüklenirken başlar
    try { console.log("tryStartGame çağrılıyor (onload sonrası). Hak kontrolü.");
        loadGameData(); // Hakları kontrol et
        if (!canPlay) { gameState = 'NO_ATTEMPTS'; requestAnimationFrame(drawGame); /* Hak bitti ekranını çiz */}
    ...
    try { if (!ctx) { return; } ctx.clearRect(0, 0, canvas.width, canvas.height);
        ...
        if (gameState === 'TUTORIAL') {
            ...
            if(feedbackMessage.text&&Date.now()<feedbackMessage.expiryTime){ ctx.fillStyle=feedbackMessage.color;ctx.font='bold 28px Arial';ctx.textAlign='center';ctx.shadowColor='black';ctx.shadowBlur=5;ctx.shadowOffsetX=2;ctx.shadowOffsetY=2;ctx.fillText(feedbackMessage.text,canvas.width/2,canvas.height-30); ctx.shadowColor='transparent';ctx.shadowBlur=0;ctx.shadowOffsetX=0;ctx.shadowOffsetY=0;ctx.textAlign='left';}else{feedbackMessage.text='';}
             ...
             // Hak bitti / Oyun bitti durumu - Mesajla gösterilecek
             console.log("Oyun Bitti veya Hak Kalmadı durumu."); gameLoopStarted = false; return;
        }

        // Döngü
        if (gameLoopStarted) { requestAnimationFrame(drawGame); } // State kontrolü kaldırıldı
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        // Başlangıç Ayarları (Dil/Bölge seçimi yok)
        loadGameData(); // Hakları kontrol etsin yeterli
