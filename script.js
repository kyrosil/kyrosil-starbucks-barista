...
function checkStartButtonState() {
    try {
        // Elementlerin varlığını fonksiyon içinde tekrar kontrol etmek daha güvenli olabilir
        const currentGsmInput = document.getElementById('gsmInput');
        const currentKvkkCheck = document.getElementById('kvkkCheck');
        const currentStartButton = document.getElementById('startButton');
        const currentGsmError = document.getElementById('gsmError');

        if (!currentGsmInput || !currentKvkkCheck || !currentStartButton) {
             console.warn("checkStartButtonState: Gerekli elementler bulunamadı.");
             return; // Elementler yoksa kontrol anlamsız
        }

        const numberEntered = currentGsmInput.value.trim().length > 0;
        const kvkkValid = currentKvkkCheck.checked;
        const assetsAreReady = bgLoaded && logoLoaded; // assetsReady global flag'ini kullan
        ...
        if (canEnable) {
            ...
            if(currentGsmError) currentGsmError.style.display = 'none';
            console.log(`checkStartButtonState: Aktif!`);
        } else {
            currentStartButton.disabled = true;
            console.log(`checkStartButtonState: Pasif!`);
            if(currentGsmError) { // Hata mesajını sadece form doldurulmaya başlandıysa ama şartlar eksikse göster
                if ( (!numberEntered || !kvkkValid) && (currentGsmInput.value.length > 0 || currentKvkkCheck.checked) ) {
                     currentGsmError.innerText = texts[currentLang]?.gsmError || "Gerekli alanları doldurun.";
                     currentGsmError.style.display = 'block';
                 } else {
                     currentGsmError.style.display = 'none';
                 }
            }
            // Buton metnini yükleniyor olarak ayarla (eğer form tamamsa)
             if(numberEntered && kvkkValid && !assetsAreReady) {
                 currentStartButton.innerText = texts[currentLang]?.loadingText || "Loading...";
             } else {
                 currentStartButton.innerText = texts[currentLang]?.startButton || "Start Game!";
             }
        }
...
function tryStartGame() {
    try { console.log("tryStartGame çağrıldı, hak kontrolü."); loadGameData();
        if (!canPlay) { gameState = 'NO_ATTEMPTS'; showMessage(texts[currentLang]?.noAttemptsTitle||"Hata", texts[currentLang]?.noAttemptsMessage||"Hak bitti.", 'error'); }
            ...
            if (!gameLoopStarted) { gameLoopStarted = true; } // Sadece flag'i ayarla
            requestAnimationFrame(drawGame); // Döngüyü başlat/tetikle
        }
...
function startGame() {
    try { console.log("startGame ÇAĞRILDI!");
        ...
        if (!assetsReady) { console.error("!!! BAŞLATMA ENGELLENDİ: Görseller yüklenmedi!"); alert("Görseller yükleniyor, butonun aktifleşmesini bekleyin."); return; }
...
function drawGame() {
    try { if (!ctx) { console.warn("drawGame: CTX yok!"); return; }
        ...
        else { ctx.fillStyle='#BBB';ctx.fillRect(0,0,canvas.width,canvas.height); ctx.fillStyle='red';ctx.font='20px Arial';ctx.textAlign='center'; ctx.fillText("ARKA PLAN HATASI/YÜKLENİYOR", canvas.width / 2, canvas.height / 2); ctx.textAlign='left'; /* Arka plan yoksa döngü devam etsin */ }
        ...
        // Diğer Çizimler
        let cTY=30;
        const currentLangTexts = texts[currentLang] || texts['EN']; // Dil yüklenemezse İngilizce'ye düş

        ctx.fillStyle='white';ctx.font='bold 16px Arial';ctx.textAlign='left'; ctx.shadowColor='black';ctx.shadowBlur=3;ctx.shadowOffsetX=1;ctx.shadowOffsetY=1; ctx.fillText(`${currentLangTexts.attemptsLeft}: ${3-failedAttemptsToday}`,20,cTY); cTY+=25; ctx.shadowColor='transparent';ctx.shadowBlur=0;ctx.shadowOffsetX=0;ctx.shadowOffsetY=0;

        if (gameState === 'TUTORIAL') { const iTS=clickableItems[tutorialStep]; if(iTS){ctx.strokeStyle=(Math.sin(Date.now()*0.005)>0)?'yellow':'orange';ctx.lineWidth=3;ctx.strokeRect(iTS.x-2,iTS.y-2,iTS.width+4,iTS.height+4); ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fillRect(0,canvas.height-60,canvas.width,60); ctx.fillStyle='white';ctx.font='bold 16px Arial';ctx.textAlign='center'; let iAT=currentLangTexts.tutorialItemAction_Default; /*...*/ ctx.fillText(currentLangTexts.tutorialItemIntro+iTS.name+iAT,canvas.width/2,canvas.height-35); ctx.font='14px Arial'; ctx.fillText(currentLangTexts.tutorialItemPrompt,canvas.width/2,canvas.height-15); ctx.textAlign='left';} else {console.warn("Öğretici adımı geçersiz:", tutorialStep); gameState='PLAYING'; currentShuffledRecipe=shuffleArray(levels[currentLevelIndex]?.clicks||[]);}}
        else if (gameState === 'PLAYING') { if(levels[currentLevelIndex]){ const d=levels[currentLevelIndex]; /*... Seviye, Sipariş, Gerekenler, Uyarılar ...*/ } else { console.warn("Geçerli seviye yok:", currentLevelIndex); } if(feedbackMessage.text&&Date.now()<feedbackMessage.expiryTime){/*... Feedback çizimi ...*/}else{feedbackMessage.text='';}}
...
// Hata logları
bgImage.onerror = () => { console.error("!!! Arka Plan YÜKLENEMEDİ! Yol: ./arka_plan.png"); bgLoaded = false; checkAssetsLoaded(); /* alert("Arka Plan PNG yüklenemedi!"); */ }
...
// loadGameData, saveGameData, getRewardForLevel, showMessage, hideMessage,
// shuffleArray, updateTexts, checkStartButtonState, checkAssetsLoaded,
// tryStartGame, startGame, drawGame, handleClick

// ÖNEMLİ: Yukarıdaki fonksiyon isimlerinin içeriklerini önceki koddan (#171)
// EKSİKSİZ ve DOĞRU BİR ŞEKİLDE kopyalayıp buraya yapıştırman gerekiyor!
// Ben sadece genel yapıyı ve DÜZELTMELERİ gösterdim.
