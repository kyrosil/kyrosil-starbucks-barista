...
// assetsReady flag'i KULLANILMAYACAK (Kullanıcının kodunda yoktu)
...
function checkStartButtonState() {
    // !!! KULLANICININ KODUNDA assetsReady KONTROLÜ YOKTU, ONU KALDIRIYORUZ !!!
    try { const numberEntered = gsmInput?.value.trim().length > 0; const kvkkValid = kvkkCheck?.checked;
        const canEnable = numberEntered && kvkkValid; // Sadece forma bak
        console.log(`checkStartButtonState: num=${numberEntered}, kvkk=${kvkkValid} => canEnable=${canEnable}`);
        if(startButton){if(canEnable){startButton.disabled=false;startButton.innerText = texts[currentLang]?.startButton || "Start Game!"; if(gsmError)gsmError.style.display='none';console.log(`checkStartButtonState: Aktif!`);}else{startButton.disabled=true;startButton.innerText = texts[currentLang]?.startButton || "Start Game!"; console.log(`checkStartButtonState: Pasif!`); if(gsmError&&(!numberEntered||!kvkkValid)&&((gsmInput&&gsmInput.value.length>0)||(kvkkCheck&&kvkkCheck.checked))){gsmError.innerText=texts[currentLang]?.gsmError||"Gerekli alanları doldurun.";gsmError.style.display='block';}else if(gsmError){gsmError.style.display='none';}}}else{console.warn("Buton yok!");}
    } catch(e){console.error("checkStartButtonState hatası:",e);}
}

// Oyunu Başlatma Tetikleyicisi (Kullanıcının kodundaki gibi - onload'dan çağrılır)
function tryStartGame() {
    // Bu fonksiyon sadece görseller yüklendiğinde çağrılacak
    // Hak kontrolü ve state ayarlaması burada yapılır.
    if (gameState !== 'LOADING' && gameState !== 'START_SCREEN') return; // Zaten başladıysa tekrar başlatma

    try { console.log("tryStartGame çağrıldı (onload sonrası). Hak kontrolü."); loadGameData();
    ...
    } catch(e){ console.error("tryStartGame hatası:",e); alert("Oyun başlatılırken hata!"); gameLoopStarted = false; }
}

// Başlat Butonu Fonksiyonu (SADECE ekranı değiştirir, oyunu tryStartGame başlatır)
function startGame() {
    try { console.log("startGame ÇAĞRILDI!");
        ...
        // Görsel yükleme kontrolü burada YOK - `tryStartGame` `onload` içinde çağrılıyor.
        ...
        // tryStartGame'i BURADA ÇAĞIRMIYORUZ! Onload çağıracak.
        // Oyun döngüsü henüz başlamadıysa, belki ilk kareyi çizdirebiliriz? Veya bekleyelim.
        console.log("Canvas gösterildi. Görseller yüklenince oyun başlayacak.");
...
function drawGame() {
    try { if (!ctx) { return; } ctx.clearRect(0, 0, canvas.width, canvas.height);
        ...
        else { ctx.fillStyle='#BBB';ctx.fillRect(0,0,canvas.width,canvas.height); ctx.fillStyle='red';ctx.font='20px Arial';ctx.textAlign='center'; ctx.fillText("ARKA PLAN HATASI/YÜKLENİYOR", canvas.width / 2, canvas.height / 2); ctx.textAlign='left'; }
        // LOGO
...
logoImage.onload = function() {
    ...
    // İki resim de yüklendiyse oyunu başlatmayı dene
    if (bgLoaded) {
        console.log("Logo yüklendi, BG zaten yüklü. tryStartGame çağrılıyor...");
       // tryStartGame(); //<<<--- BUTONLA BAŞLATACAĞIZ, BURADAN DEĞİL!
       checkStartButtonState(); // Sadece buton durumunu güncelle
    }
};
bgImage.onerror = () => { console.error("!!! Arka Plan YÜKLENEMEDİ!"); bgLoaded = false; checkStartButtonState(); alert("Arka Plan PNG yüklenemedi!"); }
