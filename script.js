...
// --- Sabit Veriler ---
// DİKKAT: texts objesi SIFIRDAN KONTROL EDİLDİ (Virgül, tırnak hataları giderildi)
const texts = {
    TR: {
        gameTitle: "Kyrosil Starbucks Barista",
        slogan: "En İyi Barista Ol! Siparişleri doğru sırayla hazırla, ödülleri kap!",
        regionLabel: "Bölge:",
        rewardTitle: "Seviye Ödülleri",
        ...
        errorTitle: "Hata!",
        errorMessage: "Yanlış malzeme veya sıra! Baştan başla.",
        winTitle: "Tebrikler!",
        ...
        closeButton: "Tamam",
        tutorialIntro: "Oyuna Hoş Geldin! İşte Tıklama Alanları:",
        tutorialItemIntro: "Bu: ",
        tutorialItemAction_Default: ". Tarifte adı geçince buraya tıklayacaksın.",
        tutorialItemAction_Cat: ". Önemli bir mola!",
        tutorialItemAction_Register: ". Siparişi bitirmek için.",
        tutorialItemAction_OrderSlip: ". Siparişi almak için.",
        tutorialItemAction_PriceList: ". Fiyatları görmek için.",
        tutorialItemAction_Fridge: ". Soğuk malzemeler burada.",
        tutorialItemAction_Dessert: ". Tatlılar burada.",
        tutorialItemPrompt: "Devam etmek için vurgulanan alana tıkla.",
        tutorialComplete: "Harika! Başlıyoruz!",
        tutorialOutOfAttempts: "Hakların bittiği için bugünlük bu kadar!",
        mixedOrderWarning: "Not: Malzemeler karışık, doğru sırada hazırla!",
        priceCheckWarning: "Müşteri fiyatı da sordu!",
        gsmLabel: "Starbucks Mobile Kayıtlı GSM No:",
        gsmPlaceholder: "Numaranızı Girin",
        ...
        rewardTypeApp: "(Starbucks App Kodu)",
        rewardTypeCash: "(NAKİT ÖDÜL!)" // Son eleman, virgül YOK
    },
    EN: {
        gameTitle: "Kyrosil Starbucks Barista",
        slogan: "Be the Best Barista! Prepare orders correctly and grab rewards!",
        regionLabel: "Region:",
        rewardTitle: "Level Rewards",
        ...
        errorTitle: "Error!",
        errorMessage: "Wrong item or sequence! Start over.",
        ...
        noAttemptsMessage: "You've used your 3 attempts for today. Come back tomorrow!",
        closeButton: "OK",
        tutorialIntro: "Welcome! Let's show you the interaction areas:",
        tutorialItemIntro: "This is the: ",
        tutorialItemAction_Default: ". Click here when it's in the recipe.",
        tutorialItemAction_Cat: ". An important break!",
        tutorialItemAction_Register: ". To finish the order.",
        tutorialItemAction_OrderSlip: ". To get the order.",
        tutorialItemAction_PriceList: ". To check prices.",
        tutorialItemAction_Fridge: ". Cold items are here.",
        tutorialItemAction_Dessert: ". Desserts are here.",
        tutorialItemPrompt: "Click the highlighted area to continue.",
        tutorialComplete: "Great! Let's begin!",
        tutorialOutOfAttempts: "You are out of attempts for today!",
        mixedOrderWarning: "Note: Ingredients listed randomly, prepare in correct order!",
        priceCheckWarning: "Customer asked for the price!",
        gsmLabel: "Starbucks Mobile Registered GSM No:",
        gsmPlaceholder: "Enter your phone number",
        ...
        rewardTypeApp: "(Starbucks App Code)",
        rewardTypeCash: "(CASH PRIZE!)" // Son eleman, virgül YOK
     }
};
...
// --- Yardımcı Fonksiyon Tanımları (Sıralama Önemli Olabilir) ---
...
// Mesajlaşma Fonksiyonları (Elementlere erişim DOMContentLoaded içinde garanti edilecek)
function showMessage(title, bodyHtml, type='info'){ try{ console.log(`showMessage Çağrıldı: Tip=${type}, Başlık=${title}`); if(!messageOverlay||!messageTitle||!messageBody) throw new Error("Mesaj elementleri hazır değil!"); messageTitle.innerText=title; messageBody.innerHTML=bodyHtml; messageOverlay.className=`overlay message-${type}`; messageOverlay.style.display='flex'; canPlay=false; } catch(e) { console.error("showMessage Hatası:", e); }}
function hideMessage(){ try{ if(!messageOverlay) throw new Error("Mesaj overlay elementi hazır değil!"); messageOverlay.style.display='none'; if(failedAttemptsToday<3 && currentLevelIndex<levels.length-1 && gameState==='PLAYING'){canPlay=true; console.log("Mesaj kapatıldı, canPlay=true.");} else { console.log("Mesaj kapatıldı, canPlay false."); canPlay = false; } } catch(e) { console.error("hideMessage Hatası:", e); }}
...
function checkAssetsLoaded(){ try{ console.log(`checkAssetsLoaded: bgLoaded=${bgLoaded}, logoLoaded=${logoLoaded}, gameState=${gameState}`); if(bgLoaded&&logoLoaded&&gameState==='LOADING'){ console.log("Görseller yüklendi, giriş ekranına geçiliyor."); gameState='START_SCREEN'; /* Artık state'i DOMContentLoaded içinde ayarlanan diğer fonksiyonlar yönetecek */ checkStartButtonState(); /* Görseller yüklenince buton durumu tekrar kontrol edilebilir */ } } catch(e){ console.error("checkAssetsLoaded Hatası:", e); } }
...
function startGame() { try { console.log("startGame ÇAĞRILDI!"); checkStartButtonState(); if (startButton.disabled) { console.warn("Başlatma engellendi."); gsmError.style.display = 'block'; return; } console.log("Başlatma Kontrolleri Geçildi.");
        if(!bgLoaded || !logoLoaded) { console.error("!!! BAŞLATMA ENGELLENDİ: Görseller henüz yüklenmedi!"); alert("Görseller hala yükleniyor, lütfen birkaç saniye bekleyin."); return; }
        ...
        tryStartGame(); } catch (e) { console.error("startGame Hatası:", e); alert("Oyun başlatılamadı!"); }}

// --- Ana Oyun Çizim Döngüsü ---
function drawGame() {
    try { if (!ctx) { console.warn("drawGame: CTX henüz hazır değil?"); return; }
        ctx.clearRect(0, 0, canvas.width, canvas.height);

        // ARKA PLAN (Kontrollü)
        if (bgLoaded && bgImage.complete && bgImage.naturalWidth > 0) { ctx.drawImage(bgImage, 0, 0, canvas.width, canvas.height); }
        else { ctx.fillStyle='#BBB';ctx.fillRect(0,0,canvas.width,canvas.height); ctx.fillStyle='red';ctx.font='20px Arial';ctx.textAlign='center'; ctx.fillText("ARKA PLAN HATASI!", canvas.width/2, canvas.height/2); ctx.textAlign='left'; if(gameLoopStarted) requestAnimationFrame(drawGame); return; } // Arka plan yoksa döngüde kal ama çizme
...
logoImage.onerror = () => { console.error("!!! Logo YÜKLENEMEDİ!"); logoLoaded = false; checkAssetsLoaded(); /* alert("Logo yüklenemedi!"); */ }
