// --- Global Değişken Tanımları (SENİN GÖNDERDİĞİN KODDAN) ---
let canvas, ctx;
let startScreenDiv, gameTitleEl, gameSloganEl, langTRButton, langENButton, regionSelect, regionLabelEl, rewardTitleEl, rewardListEl, startButton, gsmInput, kvkkCheck, gsmError, gsmLabel, kvkkLabel; // Bunlar HTML'de varsa atanacak
let messageOverlay, messageTitle, messageBody, closeButton; // Bunlar HTML'de varsa atanacak
const bgImage = new Image(); const logoImage = new Image();
let bgLoaded = false, logoLoaded = false;
let assetsReady = false; // SENİN KODUNDA VARDI, KORUYORUM
...
// --- Sabit Veriler ---
// texts objesi TAMAMEN YENİLENDİ (İngilizce + Öğe nameKey'leri dahil)
const texts = {
    TR: {
    ...
    EN: { // İngilizce Çeviriler Tamamlandı
...
// --- Yardımcı Fonksiyon Tanımları (SENİN KODUNDAN - Ufak Güncellemelerle) ---
...
// updateTexts, checkStartButtonState, startGame GİBİ GİRİŞ EKRANI FONKSİYONLARI YOK!
// (Senin kodunda bunlar DOMContentLoaded içinde tanımlanmamıştı, globaldeydiler)

// Oyunu Başlatma Tetikleyicisi (SENİN KODUNDAKİ GİBİ - onload içinden çağrılır)
function tryStartGame() {
    if (gameState !== 'LOADING') { console.warn("tryStartGame tekrar çağrıldı, state:", gameState); return; }
    try { console.log("tryStartGame çağrılıyor (onload sonrası).");
        ...
        console.log("Oyun durumu ayarlandı:", gameState, ". Döngü başlıyor...");
    ...
    try { if (!ctx) { return; } ctx.clearRect(0, 0, canvas.width, canvas.height);
        ...
        if (gameState === 'TUTORIAL') {
            ...
            if(feedbackMessage.text&&Date.now()<feedbackMessage.expiryTime){ ctx.fillStyle=feedbackMessage.color;ctx.font='bold 28px Arial';ctx.textAlign='center';ctx.shadowColor='black';ctx.shadowBlur=5;ctx.shadowOffsetX=2;ctx.shadowOffsetY=2;ctx.fillText(feedbackMessage.text,canvas.width/2,canvas.height-30); ctx.shadowColor='transparent';ctx.shadowBlur=0;ctx.shadowOffsetX=0;ctx.shadowOffsetY=0;ctx.textAlign='left';}else{feedbackMessage.text='';}
             ...
             // Bu durumlar showMessage ile hallediliyor.
             console.log("Oyun Bitti veya Hak Kalmadı durumu (Mesaj gösteriliyor olmalı)."); gameLoopStarted = false; return;
        }
...
// Tıklama İşleyici Fonksiyon (GÜNCELLENDİ - nameKey ve touch)
...
// --- GÖRSEL YÜKLEME OLAYLARI (KULLANICININ İSTEDİĞİ GİBİ - tryStartGame Çağırır) ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        if (!canvas||!ctx||!messageOverlay||!closeButton) { throw new Error("Kritik HTML elementleri bulunamadı!"); }
        console.log("Temel element referansları alındı.");

        // Olay Dinleyicileri (Canvas ve Mesaj Kapatma)
        // Dil/Bölge/Start Butonu listener'ları YOK
        if(closeButton) closeButton.addEventListener('click', hideMessage); else console.warn("closeButton DOM'da yok");
        if(canvas) {
            canvas.addEventListener('click', (event) => { processClickOrTouch(event.offsetX, event.offsetY); });
            canvas.addEventListener('touchstart', handleTouchStart, { passive: false });
        } else { console.error("Canvas DOM'da yok, listener eklenemedi"); }
        ...
        // Başlangıç Ayarları (Minimal)
        loadGameData(); // Sadece hakları kontrol etsin
