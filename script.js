// --- Global Değişkenler ---
// Element Referansları (DOM Yüklendikten sonra atanacak)
let canvas, ctx, startScreenDiv, gameTitleEl, gameSloganEl, langTRButton, langENButton, regionSelect, regionLabelEl, rewardTitleEl, rewardListEl, startButton, gsmInput, kvkkCheck, gsmError, gsmLabel, kvkkLabel, messageOverlay, messageTitle, messageBody, closeButton;
...
// Oyun Değişkenleri
let currentLevelIndex = 0; let currentRecipeStep = 0; let canPlay = true; let gameLoopStarted = false; let currentShuffledRecipe = [];
// Dil ve Bölge
...
// --- Yardımcı Fonksiyon Tanımları (Fonksiyonlar DOMContentLoaded'den ÖNCE tanımlanmalı) ---
...
function showMessage(title, bodyHtml, type='info'){ try{ console.log(`showMessage Çağrıldı: Tip=${type}, Başlık=${title}`); if(!messageOverlay||!messageTitle||!messageBody) throw new Error("Mesaj elementleri bulunamadı!"); messageTitle.innerText=title; messageBody.innerHTML=bodyHtml; messageOverlay.className=`overlay message-${type}`; messageOverlay.style.display='flex'; canPlay=false; } catch(e) { console.error("showMessage Hatası:", e); }}
function hideMessage(){ try{ if(!messageOverlay) throw new Error("Mesaj overlay elementi bulunamadı!"); messageOverlay.style.display='none'; // Her durumda mesajı kapat
    // Sadece oyun devam ediyorsa ve hak varsa oynamaya izin ver
    if(gameState === 'PLAYING' && failedAttemptsToday<3 && currentLevelIndex<levels.length-1){canPlay=true; console.log("Mesaj kapatıldı, canPlay=true (Oyun Devam).");}
    else { console.log("Mesaj kapatıldı, canPlay false kalıyor veya oyun bitti/hak bitti."); canPlay = false; /* Oyun bittiyse veya hak yoksa kapalı kalsın */ } } catch(e) { console.error("hideMessage Hatası:", e); }}
...
function tryStartGame() {
    try { console.log("tryStartGame çağrıldı, hak kontrolü."); loadGameData();
        if (!canPlay) { gameState = 'NO_ATTEMPTS'; showMessage(texts[currentLang]?.noAttemptsTitle||"Hata", texts[currentLang]?.noAttemptsMessage||"Hak bitti.", 'error'); }
            ...
            currentLevelIndex = 0; currentRecipeStep = 0; // Oyuna her zaman seviye 1'den başla
...
function startGame() { try { console.log("startGame ÇAĞRILDI!"); checkStartButtonState(); if (startButton.disabled) { console.warn("Başlatma engellendi."); gsmError.style.display = 'block'; return; } console.log("Başlatma Kontrolleri Geçildi."); if(startScreenDiv)startScreenDiv.style.display = 'none'; else throw new Error("startScreenDiv null!"); if(canvas)canvas.style.display = 'block'; else throw new Error("canvas null!"); tryStartGame(); } catch (e) { console.error("startGame Hatası:", e); alert("Oyun başlatılamadı!"); }}

function drawGame() {
    try { if (!ctx) { console.error("drawGame: CTX geçerli değil!"); return; }
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        // Arka Plan
        if (bgLoaded && bgImage.complete && bgImage.naturalWidth > 0) { ctx.drawImage(bgImage, 0, 0, canvas.width, canvas.height); }
        else { ctx.fillStyle = '#DDD'; ctx.fillRect(0, 0, canvas.width, canvas.height); /* Fallback */ }
        // Logo
        ...
        if (gameState === 'TUTORIAL') { const iTS=clickableItems[tutorialStep]; if(iTS){ctx.strokeStyle=(Math.sin(Date.now()*0.005)>0)?'yellow':'orange';ctx.lineWidth=3;ctx.strokeRect(iTS.x-2,iTS.y-2,iTS.width+4,iTS.height+4); ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fillRect(0,canvas.height-60,canvas.width,60); ctx.fillStyle='white';ctx.font='bold 16px Arial';ctx.textAlign='center'; let iAT=texts[currentLang]?.tutorialItemAction_Default||". Click."; if(iTS.name==='Tezgahtaki Kedi')iAT=texts[currentLang].tutorialItemAction_Cat; else if(iTS.name==='Kasa')iAT=texts[currentLang].tutorialItemAction_Register; else if(iTS.name==='Sipariş Fişi')iAT=texts[currentLang].tutorialItemAction_OrderSlip; else if(iTS.name==='Fiyat Listesi')iAT=texts[currentLang].tutorialItemAction_PriceList; else if(iTS.name==='Buzdolabı')iAT=texts[currentLang].tutorialItemAction_Fridge; else if(iTS.name==='Tatlı Dolabı')iAT=texts[currentLang].tutorialItemAction_Dessert; ctx.fillText((texts[currentLang]?.tutorialItemIntro||"This:")+iTS.name+iAT,canvas.width/2,canvas.height-35); ctx.font='14px Arial'; ctx.fillText(texts[currentLang]?.tutorialItemPrompt||"Click area.",canvas.width/2,canvas.height-15); ctx.textAlign='left';} else {console.warn("Öğretici adımı geçersiz:", tutorialStep); gameState='PLAYING'; currentShuffledRecipe=shuffleArray(levels[currentLevelIndex]?.clicks||[]);}}
        ...
        // Diğer game state'ler mesajla hallediliyor.
        // Döngüyü devam ettir
        if (gameLoopStarted && (gameState === 'PLAYING' || gameState === 'TUTORIAL')) { requestAnimationFrame(drawGame); }
        else { console.log("Oyun döngüsü durdu/devam etmiyor. State:", gameState); gameLoopStarted = false;}
    } catch (e) { console.error("Draw HATA:", e); gameLoopStarted = false; /* alert("Çizim hatası!"); */ }
}

// Tıklama İşleyici Fonksiyon
function handleClick(event) {
    try{ if (messageOverlay?.style.display === 'flex') return; // Mesaj açıksa işlem yapma
