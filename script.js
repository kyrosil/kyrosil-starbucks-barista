...
let assetsReady = false; // Görsellerin yüklendiğini belirten bayrak
...
function checkStartButtonState() {
    try {
        ...
        // Butonun aktifleşmesi için görsellerin de yüklenmiş olması LAZIM
        const canEnable = numberEntered && kvkkValid && assetsReady; // assetsReady kontrolü DÜZELTİLDİ
        console.log(`checkStartButtonState: num=${numberEntered}, kvkk=${kvkkValid}, assetsReady=${assetsReady} => canEnable=${canEnable}`);

        if (startButton) {
            if (canEnable) {
                startButton.disabled = false;
                startButton.innerText = texts[currentLang]?.startButton || "Start Game!"; // Yükleniyor yazısını kaldır
                ...
                if (numberEntered && kvkkValid && !assetsReady) { // Form tamam ama yükleniyor
                     startButton.innerText = texts[currentLang]?.loadingText || "Loading...";
                     if(gsmError) gsmError.style.display = 'none'; // Yükleniyor derken hata gösterme
                } else { // Form eksik
...
// Görsel Yükleme Kontrolü (Düzeltildi)
function checkAssetsLoaded(){
    try{
        console.log(`checkAssetsLoaded: bgLoaded=${bgLoaded}, logoLoaded=${logoLoaded}, assetsReady=${assetsReady}`);
        if (bgLoaded && logoLoaded && !assetsReady) { // assetsReady false ise ve ikisi de yüklendiyse
            ...
            console.log(">>> assetsReady = true yapıldı. Buton durumu KONTROL EDİLİYOR...");
...
function tryStartGame() {
    try { console.log("tryStartGame çağrıldı."); loadGameData();
        if (!canPlay) { gameState = 'NO_ATTEMPTS'; showMessage(texts[currentLang]?.noAttemptsTitle||"Hata", texts[currentLang]?.noAttemptsMessage||"Hak bitti.", 'error'); }
            ...
            if (!gameLoopStarted) { gameLoopStarted = true; requestAnimationFrame(drawGame); } else { requestAnimationFrame(drawGame); }
        }
...
function startGame() {
    try { console.log("startGame ÇAĞRILDI!");
        ...
        // Buton aktifse, görseller yüklenmiş demektir (checkStartButtonState'den dolayı).
        if (!assetsReady) { console.error("HATA: assetsReady false iken startGame çağrıldı!"); return; } // Ekstra güvenlik
...
function drawGame() {
    try { if (!ctx) { return; } // Context yoksa çizme
        ...
        if (bgLoaded && bgImage.complete && bgImage.naturalWidth > 0) {
            ...
            ctx.fillStyle='#BBB'; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.fillStyle='red'; ctx.font='20px Arial'; ctx.textAlign='center'; ctx.fillText("ARKA PLAN YÜKLENEMEDİ/BEKLENİYOR", canvas.width / 2, canvas.height / 2); ctx.textAlign='left';
        }
        ...
        // Diğer Çizimler
        let cTY=30;
        const langText = texts[currentLang] || texts['TR']; // Dil yüklenemezse TR'ye düş
