...
function hideMessage(){ try{ if(!messageOverlay) throw new Error("Mesaj overlay elementi bulunamadı!"); messageOverlay.style.display='none'; if(failedAttemptsToday<3 && currentLevelIndex<levels.length-1 && (gameState==='PLAYING' || gameState==='TUTORIAL' /* Tutorial'dan sonra da devam edebilmeli */)){canPlay=true; console.log("Mesaj kapatıldı, canPlay=true yapıldı."); } else { console.log("Mesaj kapatıldı, canPlay false kalıyor veya oyun bitti.");} } catch(e) { console.error("hideMessage Hatası:", e); }}
...
function updateTexts(lang, region) {
    try { console.log(`updateTexts Başladı: Lang=${lang}, Region=${region}`); const t = texts[lang]; if (!t) throw new Error(`Metinler bulunamadı: ${lang}`);
        document.title = t.gameTitle; gameTitleEl.innerText = t.gameTitle; gameSloganEl.innerText = t.slogan; regionLabelEl.innerText = t.regionLabel; rewardTitleEl.innerText = t.rewardTitle; startButton.innerText = t.startButton; closeButton.innerText = t.closeButton; gsmLabel.innerText = t.gsmLabel; kvkkLabel.innerHTML = t.kvkkLabel; gsmError.innerText = t.gsmError; if(gsmInput) gsmInput.placeholder = t.gsmPlaceholder;
        // Ödül Listesi
        ...
        langTRButton.classList.toggle('active', lang === 'TR'); langENButton.classList.toggle('active', lang === 'EN'); document.documentElement.lang = lang.toLowerCase(); console.log(`Metinler ${lang} (${region}) güncellendi.`);
    } catch (error) { console.error("updateTexts hatası:", error); }
}

function checkStartButtonState() { try { const numberEntered = gsmInput?.value.trim().length > 0; const kvkkValid = kvkkCheck?.checked; console.log(`checkStartButtonState: num=${numberEntered}, kvkk=${kvkkValid}`); if(startButton){if(numberEntered&&kvkkValid){startButton.disabled=false;if(gsmError)gsmError.style.display='none';console.log(`checkStartButtonState: Aktif!`);}else{startButton.disabled=true;console.log(`checkStartButtonState: Pasif!`); if(gsmError&&(!numberEntered||!kvkkValid)&&((gsmInput&&gsmInput.value.length>0)||(kvkkCheck&&kvkkCheck.checked))){gsmError.innerText=texts[currentLang]?.gsmError||"Gerekli alanları doldurun.";gsmError.style.display='block';}else if(gsmError){gsmError.style.display='none';}}}else{console.warn("Buton yok!");}}catch(e){console.error("checkStartButtonState hatası:",e);} }

function checkAssetsLoaded(){ try{ if(bgLoaded&&logoLoaded&&gameState==='LOADING'){ console.log("Görseller yüklendi, başlangıç ekranına geçiliyor."); gameState='START_SCREEN'; loadGameData(); updateTexts(currentLang,currentRegion); checkStartButtonState(); } } catch(e) { console.error("checkAssetsLoaded Hatası:", e); } }

function tryStartGame() {
    try { console.log("tryStartGame çağrıldı, hak kontrolü."); loadGameData(); // Hakları tekrar kontrol et
        if (!canPlay) { gameState = 'NO_ATTEMPTS'; console.log("Hak yok, durum NO_ATTEMPTS yapıldı."); showMessage(texts[currentLang]?.noAttemptsTitle||"Hata", texts[currentLang]?.noAttemptsMessage||"Hak bitti.", 'error'); }
            ...
            if (!gameLoopStarted) { gameLoopStarted = true; } // Başladığını işaretle
            requestAnimationFrame(drawGame); // Döngüyü başlat/devam et
        }
    } catch(e){ console.error("tryStartGame hatası:",e); alert("Oyunu başlatırken bir hata oluştu!"); }
}
...
function drawGame() {
    try { if(!ctx){console.error("drawGame: CTX yok!"); return;}
        ctx.clearRect(0,0,canvas.width,canvas.height); if(bgLoaded) ctx.drawImage(bgImage,0,0,canvas.width,canvas.height); else {ctx.fillStyle='lightgrey'; ctx.fillRect(0,0,canvas.width,canvas.height); /* Yükleniyor...*/}
        if(logoLoaded){logoX=canvas.width/2-logoWidth/2; const cX=logoX+logoWidth/2; const cY=logoY+logoHeight/2; const r=logoWidth/2; ctx.fillStyle='white'; ctx.beginPath(); ctx.arc(cX,cY,r,0,Math.PI*2); ctx.fill(); ctx.drawImage(logoImage,logoX,logoY,logoWidth,logoHeight);}
        let currentTextY = 30; ctx.fillStyle='white';ctx.font='bold 16px Arial';ctx.textAlign='left'; ctx.shadowColor='black';ctx.shadowBlur=3;ctx.shadowOffsetX=1;ctx.shadowOffsetY=1; ctx.fillText(`${texts[currentLang]?.attemptsLeft||'Attempts Left'}: ${3-failedAttemptsToday}`,20,cTY); currentTextY+=25; ctx.shadowColor='transparent';ctx.shadowBlur=0;ctx.shadowOffsetX=0;ctx.shadowOffsetY=0;

        if (gameState === 'TUTORIAL') {
            const itemToShow=clickableItems[tutorialStep]; if(itemToShow){ctx.strokeStyle=(Math.sin(Date.now()*0.005)>0)?'yellow':'orange';ctx.lineWidth=3;ctx.strokeRect(itemToShow.x-2,itemToShow.y-2,itemToShow.width+4,itemToShow.height+4); ctx.fillStyle='rgba(0,0,0,0.7)';ctx.fillRect(0,canvas.height-60,canvas.width,60); ctx.fillStyle='white';ctx.font='bold 16px Arial';ctx.textAlign='center'; let iAT=texts[currentLang]?.tutorialItemAction_Default||". Click."; /*...*/ ctx.fillText((texts[currentLang]?.tutorialItemIntro||"This:")+itemToShow.name+iAT,canvas.width/2,canvas.height-35); ctx.font='14px Arial'; ctx.fillText(texts[currentLang]?.tutorialItemPrompt||"Click area.",canvas.width/2,canvas.height-15); ctx.textAlign='left';} else {console.warn("Öğretici adımı geçersiz:", tutorialStep);}
        } else if (gameState === 'PLAYING') {
            if(levels[currentLevelIndex]){ const d=levels[currentLevelIndex]; ctx.fillStyle = 'white'; ctx.textAlign = 'left'; ctx.shadowColor = 'black'; ctx.shadowBlur = 4; ctx.shadowOffsetX = 2; ctx.shadowOffsetY = 2; ctx.font='bold 20px Arial'; ctx.fillText(`${texts[currentLang]?.level||'Lvl'}: ${d.level}`,20,currentTextY); currentTextY+=25; if(d.clicks.length>0){ctx.font='18px Arial'; ctx.fillText(`${texts[currentLang]?.order||'Order'}: ${d.recipeName}`,20,currentTextY); currentTextY+=25; ctx.font='italic 16px Arial'; ctx.fillText(`${texts[currentLang]?.requirements||'Required'}:`,20,currentTextY); currentTextY+=20; const sC=currentShuffledRecipe; for(const i of sC){ctx.fillText(`- ${i}`,30,currentTextY); currentTextY+=18;} currentTextY+=5;ctx.fillStyle = 'orange'; ctx.font = 'bold 14px Arial'; ctx.fillText(texts[currentLang]?.mixedOrderWarning||"Note: Prepare in order!",20,currentTextY); currentTextY+=20; if(d.clicks.includes('Fiyat Listesi')){ctx.fillStyle = 'lightblue';ctx.fillText(texts[currentLang]?.priceCheckWarning||"Price check!",20,currentTextY);currentTextY+=20;} } ctx.shadowColor='transparent';ctx.shadowBlur=0;ctx.shadowOffsetX=0;ctx.shadowOffsetY=0;} else { console.warn("Geçerli seviye yok:", currentLevelIndex); }
        ...
        // Hak Bitti / Oyun Bitti durumları artık HTML overlay ile gösteriliyor.

        // Döngüyü devam ettir
        if (gameLoopStarted && (gameState === 'PLAYING' || gameState === 'TUTORIAL')) { requestAnimationFrame(drawGame); }
        else { console.log("Oyun döngüsü durdu. State:", gameState); gameLoopStarted = false; } // Döngü durunca flag'i sıfırla

    } catch (e) { console.error("Draw HATA:", e); gameLoopStarted = false; alert("Çizim sırasında bir hata oluştu!"); }
}


function handleClick(event) {
    try{
        // Mesaj kutusu açıksa tıklama işlemiyor
        if (messageOverlay && messageOverlay.style.display === 'flex') return;

        const rect = canvas.getBoundingClientRect(); const clickX = event.offsetX; const clickY = event.offsetY; // event.offsetX/Y daha doğru olabilir
        ...
        if (gameState === 'TUTORIAL') {
            ...
            if (!canPlay || currentLevelIndex >= levels.length - 1) return; let clickedItemName=null; for(const item of clickableItems){if(clickX>=item.x&&clickX<=item.x+item.width&&clickY>=item.y&&clickY<=item.y+item.height){clickedItemName=item.name;break;}} if(clickedItemName){console.log(`Oyun: Tıklandı: ${clickedItemName}`);const currentLevelData=levels[currentLevelIndex];const expectedClick=currentLevelData.clicks[currentRecipeStep];if(clickedItemName===expectedClick){console.log("Oyun: Doğru adım!");currentRecipeStep++;if(currentRecipeStep>=currentLevelData.clicks.length){const cL=currentLevelData.level;console.log(`--- Seviye ${cL} Bitti! ---`);const rAS=getRewardForLevel(cL,currentRegion);currentLevelIndex++;currentRecipeStep=0;if(levels[currentLevelIndex]?.clicks.length>0){currentShuffledRecipe=shuffleArray(levels[currentLevelIndex].clicks);}else{currentShuffledRecipe=[];} if(rAS){console.warn(`%cÖDÜL! Seviye ${cL} (${rAS})`,'color:green;font-weight:bold;');const isL10=cL===10;const winMsgPart2=isL10?texts[currentLang].winMessagePart2_USDT:texts[currentLang].winMessagePart2_App.replace('{REWARD}',rAS);const mBB=isL10?texts[currentLang].winMessageEmailBodyBase_USDT:texts[currentLang].winMessageEmailBodyBase_App;const mB=encodeURIComponent(mBB.replace('{LEVEL}',cL).replace('{REWARD}',rAS));const mS=encodeURIComponent(`${texts[currentLang].winMessageEmailSubjectBase}${cL}${isL10?' - NAKIT ODUL':''}`);const mTL=`mailto:${texts[currentLang].winMessageEmailAddress}?subject=${mS}&body=${mB}`;const wH=`<p>${texts[currentLang].winMessagePart1}${cL}${winMsgPart2}</p><hr><p>${texts[currentLang].winMessageEmailPrompt}<br><a href="${mTL}" target="_blank"><b>${texts[currentLang].winMessageEmailAddress}</b></a><br>${texts[currentLang].winMessageEmailInstructions}</p>`;showMessage(texts[currentLang].winTitle,wH,'win');} const nLD=levels[currentLevelIndex];if(!nLD||nLD.clicks.length===0){console.log("OYUN TAMAMLANDI!");gameState='GAME_OVER';showMessage(texts[currentLang].gameOverTitle,texts[currentLang].gameOverMessage,'info');}}else{/*Doğru adım feedback?*/}}else{console.log("Oyun: Yanlış! Baştan başla.");currentRecipeStep=0;failedAttemptsToday++;saveGameData();console.log(`Kalan hak: ${3-failedAttemptsToday}/3`);feedbackMessage={text:texts[currentLang]?.errorMessage||"Wrong!",color:'red',expiryTime:Date.now()+2500};if(failedAttemptsToday>=3){canPlay=false;console.error("Hak bitti!");gameState='NO_ATTEMPTS';setTimeout(()=>{if(!canPlay){showMessage(texts[currentLang]?.noAttemptsTitle||"Attempts Done",texts[currentLang]?.noAttemptsMessage||"Try tomorrow",'error');}},2550);}}}else{console.log("Oyun: Boş alan tıklandı.");}}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        langTRButton.addEventListener('click', () => { if(currentLang!=='TR'){currentLang='TR';updateTexts(currentLang, currentRegion);localStorage.setItem('barista_lang', currentLang);}}); // Dil tercihini kaydet
        langENButton.addEventListener('click', () => { if(currentLang!=='EN'){currentLang='EN';updateTexts(currentLang, currentRegion);localStorage.setItem('barista_lang', currentLang);}});
        regionSelect.addEventListener('change', (event) => { currentRegion=event.target.value; updateTexts(currentLang, currentRegion); localStorage.setItem('barista_region', currentRegion);}); // Bölge tercihini kaydet
